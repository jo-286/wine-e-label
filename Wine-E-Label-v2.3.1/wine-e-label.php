...
define('WINE_E_LABEL_VERSION', '2.3.2');
define('WINE_E_LABEL_DB_VERSION', '1.71');
define('WINE_E_LABEL_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('WINE_E_LABEL_PLUGIN_URL', plugin_dir_url(__FILE__));
...
if (!defined('WINE_E_LABEL_URL_PREFIX')) {
  define('WINE_E_LABEL_URL_PREFIX', 'l');
}
if (!defined('WINE_E_LABEL_SHORTCODE_LENGTH')) {
  define('WINE_E_LABEL_SHORTCODE_LENGTH', 5);
}
if (!defined('WINE_E_LABEL_CHARACTER_SET')) {
  define('WINE_E_LABEL_CHARACTER_SET', 'alphanumeric');
}

require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-ingredients.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-db-extended.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-importer.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-manual-builder.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-design.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-presentation.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-url.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-frontend.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-elementor.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-admin-i18n.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-migration.php';
require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-github-updater.php';
...
class Wine_E_Label_Plugin
{
  public function __construct()
  {
    new Wine_E_Label_GitHub_Updater(array(
      ...
      'plugin_version'  => WINE_E_LABEL_VERSION,
    ...
    Wine_E_Label_Admin_I18n::init();
    add_action('plugins_loaded', [$this, 'load_textdomain']);
    add_action('init', [$this, 'run_internal_key_migrations'], 0);
  ...
  {
    Wine_E_Label_Migration::run();
    Wine_E_Label_URL::init();
    Wine_E_Label_Frontend::init();
    Wine_E_Label_Elementor::init();

    if (is_admin()) {
      require_once WINE_E_LABEL_PLUGIN_DIR . 'admin/working-metabox.php';
      require_once WINE_E_LABEL_PLUGIN_DIR . 'admin/class-wine-e-label-admin-extended.php';
      require_once WINE_E_LABEL_PLUGIN_DIR . 'includes/class-wine-e-label-qr.php';
      $db = new Wine_E_Label_DB_Extended();
      new Working_Wine_E_Label_MetaBox($db);
      new Wine_E_Label_Admin_Extended();
    }
  }
  ...
  public static function migrate_database()
  {
    ...
    $installed_db_version = get_option('wine_e_label_db_version', '0.0.0');

    if (version_compare($installed_db_version, '1.2.0', '<')) {
      $wpdb->query("ALTER TABLE {$table} MODIFY short_code VARCHAR(120) NOT NULL");
      update_option('wine_e_label_db_version', '1.2.0');
    }
  }

  public function run_internal_key_migrations()
  {
    Wine_E_Label_Migration::run();
  }
  ...
  public function maybe_cleanup_duplicate_directories()
  {
    ...
    $current_dir = basename(WINE_E_LABEL_PLUGIN_DIR);
