...
class NutritionLabels_DB_Extended
{
  ...
  public function shortcode_belongs_to_other_product(string $shortcode, int $product_id): bool
  {
    if (!$this->is_valid_shortcode($shortcode)) {
      return true;
    }
    ...
    return $existing !== null && (int) $existing !== $product_id;
  }

  public function upsert_shortcode(int $product_id, string $shortcode)
  {
    if (!is_numeric($product_id) || $product_id <= 0) {
      return false;
    }
    ...
    if ($this->shortcode_belongs_to_other_product($shortcode, $product_id)) {
      return false;
    }
    ...
    return $this->wpdb->update(
      $this->table_name,
      ['url_prefix' => '', 'short_code' => $shortcode, 'updated_at' => current_time('mysql')],
      ['product_id' => $product_id],
      ['%s', '%s', '%s'],
      ['%d']
    );
  ...
  private function is_valid_shortcode($shortcode)
  {
    ...
    if (!preg_match('/^[a-z0-9-]{1,120}$/', $shortcode)) {
      return false;
    }
